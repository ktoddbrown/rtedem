...
par <- list(v_enz=5, km_enz=2,
                ...
                turnover_e=0.005,
                dormancy_rate=0.01,
                sorb_rate=0.01, desorb_rate=0.001, carbonCapacity=5)
pools <- list(simple=0.1, dormant=10, biomass=1, enzyme=0.01, protect_S=1, protect_C=3,  complex=10)
...
poolOuts <- runModel(par=parFull, timeArr=10^seq(-1, 2, by=0.1)*365, cModel=dC.biomassModel, C_bulk=totalC, dt=1, poolAssignment=poolIndex, rateFlags=list(enz='MM', uptake='MM', prod='uptake', sorb='carryCapacity'), allocationStr='a')
poolOuts <- melt(poolOuts, id.vars=c('time', 'label'))
ggplot(poolOuts) + geom_line(aes(x=time/365, y=value))  + scale_x_log10() + labs(title='Young carbon pools', x='time [year]', y='C stock')+ facet_wrap(~variable, scales='free')

CO2Outs <- createSynData(par=parFull, timeArr=10^seq(-1, 2, by=0.1)*365, cModel=dC.biomassModel, C_bulk=totalC, dt=1, poolAssignment=poolIndex, rateFlags=list(enz='MM', uptake='MM', prod='uptake', sorb='carryCapacity'), allocationStr='a')

ggplot(CO2Outs) + geom_line(aes(x=time/365, y=mean)) + geom_errorbar(aes(x=time/365, ymin=mean-sd, ymax=mean+sd)) + scale_x_log10() +  labs(title='Expected respiration', x='time [year]', x='CO2 flux [g CO2-C g^-1 soil-C day^-1]')
```