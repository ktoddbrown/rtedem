...
makePlots <- function(par, pools, timeArr=10^seq(-1, 0, length=20)*365){
  ...
  ans <- list(pools = ggplot(poolOuts) + geom_line(aes(x=time, y=value))  + scale_x_log10() + labs(title='Carbon pools', x='time [day]', y='C stock')+ facet_wrap(~variable, scales='free'))
  ...
  ans$CO2 <- ggplot(CO2Outs) + geom_line(aes(x=time, y=mean)) + geom_errorbar(aes(x=time, ymin=mean-sd, ymax=mean+sd)) + scale_x_log10() +  labs(title='Synthetic respiration', x='time [day]', y='CO2 flux [g CO2-C g^-1 soil-C day^-1]')
