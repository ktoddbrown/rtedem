...
One major goal of *rtedem* is to streamline parameter estimation in soil decomposition studies. In this vingette we will walk through the generation of synthetic data to do study development, use *FME::modMCMC* to back out the associated parameters, and some basic visualizations.


First we import some useful libraries included *rtedem*.
```{r}
library(rtedem)
library(ggplot2)
library(FME)
...
Next we set up the parameters required to generate our synthetic data, generate the synthetic data, and do some basic visualizations.
```{r setUpParameters, fig.width=6, cache=TRUE}
...
Then we run MCMC using a 'guess' based on the parameters defined above to try to back out the parameters used to generate our synthetic data.

```{r runMCMC, fig.width=4, cache=TRUE}
...
Finally some basic visulaizations looking at how the MCMC runs fit the data and what the final parameter distributions are.
